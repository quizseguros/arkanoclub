...
export default function ProductCard({ product, glow }: { product: Product; glow?: boolean }) {
  ...
  const inner = (
    <div
      className={`group flex flex-col overflow-hidden bg-arkano-graphite transition ${
        glow
          ? "h-full rounded-[11px]"
          : "rounded-xl border border-white/10 hover:border-arkano-gold/40"
      }`}
    >
  ...
  if (glow) {
    return (
      <div className="product-glow-border rounded-xl p-px">
        {inner}
      </div>
    );
  }
